...
public class DepositaPreleva extends JFrame {
	...
	private void actions(String filePath, JPanel panel, String soldiString, int settimana, double portafoglio,
		...
		if (depositaPreleva.equals("Deposita")) {
			depositoPrelievo(filePath, panel, giusto, settimana, portafoglio, contoCorrente);
		} else {
			depositoPrelievo(filePath, panel, giusto, settimana, contoCorrente, portafoglio);
		}
	}
	 ...
	 *                  = portafoglio)
	 */
	private void depositoPrelievo(String filePath, JPanel panel, boolean giusto, int settimana, double togli,
			double aggiungi) {
		if (!giusto || (soldi <= 0 || soldi > togli)) {
			...
			double nuovoPortafoglio, nuovoConto;

			double[] dati = sommaSottrazione(aggiungi, togli, soldi);
			if (depositaPreleva.equals("Deposita")) {
				nuovoPortafoglio = dati[0];
				nuovoConto = dati[1];
			} else {
				nuovoPortafoglio = dati[1];
				nuovoConto = dati[0];
			}

			try {
				FileTools.aggiungiNuovaRiga(filePath, nuovoPortafoglio, nuovoConto, settimana, "Deposito");
