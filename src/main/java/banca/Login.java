...
public class Login extends JFrame {
  ...
  /**
   * Metodo per accedere a un file che ha come nome utente la stringa indicata nel campo userText.
   * Qui viene controllato se la password coincide e se l'account non e' in esecuzione. Se si' viene
   * modificata la prima riga del file, impostando il flag a 1 per indicare che l'account e' in
   * esecuzione. Se no mostra un messaggio di errore
   *
   * @param panel -- JPanel al quale si collega la finestra per mandare messaggi
   * @param filePath -- Percorso del file interessato
   * @param utente -- variabile File per controllare la password presente al suo interno
    ...
    try (BufferedReader br = new BufferedReader(new FileReader(utente))) {
      ...
      if (open.equals("0") && passwordUtente.equals(password)) {
        ...
        JOptionPane.showMessageDialog(
            panel, "Password errata o account in esecuzione.", "Errore", JOptionPane.ERROR_MESSAGE);
      }
  ...
  /**
   * Metodo che crea un file per un nuovo utente contenente: una prima riga con password e flag a 1
   * che indica che l'account e' in esecuzione e una seconda riga con saldo portafoglio iniziale
   * (100.0), saldo conto corrente iniziale (tra 10 e 75), 1 per dire che e' la prima settimana e la
   * parola "Creazione" per indicare che è il momento in cui e' stato creato l'account
   *
   * @param panel -- JPanel al quale si collega la finestra per mandare messaggi
