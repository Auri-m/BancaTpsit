...
public class Login extends JFrame {
		...
		loginButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (utente.exists()) {
					...
					creazione(panel, utente, password);
				}
	 ...
	 */
	private void creazione(JPanel panel, File utente, String password) {
		try {
			...
			try (BufferedWriter writer = new BufferedWriter(new FileWriter(utente))) {
				...
				writer.newLine();
				writer.write(1 + ";" + FileTools.GeneraRandom(75, 10) + ";" + 100.0 + ";" + "Creazione");
